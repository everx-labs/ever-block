...
pub trait HashmapAugType<K: Deserializable + Serializable, X: Deserializable + Serializable, Y: Augmentable>: HashmapType {
    ...
    where F: FnMut(SliceData, Y, Option<X>) -> Result<TraverseNextStep<R>> {
        self.traverse_slices(|key, mut label| {
            let aug = Y::construct_from(&mut label)?;
            if key.remaining_bits() == self.bit_len() {
    ...
    where F: FnMut(SliceData, SliceData) -> Result<TraverseNextStep<R>> {
        if let Some(root) = self.data() {
    ...
    where F: FnMut(SliceData, SliceData) -> Result<crate::hashmapaug::TraverseNextStep<R>> {
        let label = cursor.get_label(bit_len)?;
        let label_length = label.remaining_bits();
        if label_length < bit_len {
            bit_len -= label_length + 1;
            ...
            aug.checked_drain_reference()?;
            key.checked_append_references_and_data(&label)?;
            match callback(key.clone().into(), aug)? {
                TraverseNextStep::Stop => return Ok(None),
                TraverseNextStep::End(r) => return Ok(Some(r)),
                cont => {
                    let (visit_first, visit_second) = match cont {
                       TraverseNextStep::VisitZero => (0, None),
                       TraverseNextStep::VisitOne => (1, None),
                       TraverseNextStep::VisitZeroOne => (0, Some(1)),
                       TraverseNextStep::VisitOneZero => (1, Some(0)),
                       _ => unreachable!()
                    };
                    {
                        let mut key = key.clone();
                        key.append_bit_bool(visit_first != 0)?;
                        let ref mut child = SliceData::from(cursor.reference(visit_first)?);
                        if let Some(r) = Self::traverse_internal(child, key, bit_len, callback)? {
                            return Ok(Some(r))
                        }
                    }
                    if let Some(visit_second) = visit_second {
                        key.append_bit_bool(visit_second != 0)?;
                        let ref mut child = SliceData::from(cursor.reference(visit_second)?);
                        if let Some(r) = Self::traverse_internal(child, key, bit_len, callback)? {
                            return Ok(Some(r))
                        }
                    }
            ...
            key.checked_append_references_and_data(&label)?;
            return match callback(key.into(), cursor.clone())? {
